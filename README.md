...
│       ├── news-archive/        # 上限を超えた古いニュース（年別・サイトからは未参照）
│       │   └── 2026.json
...
ニュースは1日およそ40件増えるため、上限を設けないと `news.json` は年に10MB前後まで育つ。`/news` はこのファイルをブラウザから丸ごと取得するので、閲覧者の通信量に直接効いてくる。そのため **新しい順に2000件（約1.3MB）だけを `news.json` に残し、あふれたぶんは `public/data/news-archive/<年>.json` へ退避する**（データは失わない。サイトからは参照していない）。上限は `NEWS_MAX_ARTICLES` で変更できる。
...
概要文はカードに表示するため、`scripts/lib/news-text.mjs` で元ページのグローバルナビ・関連記事欄・著作権表記を落としてから保存する。中黒などで区切られた短い断片をメニュー項目とみなして捨て、「ちいかわ」を含むか20文字以上の断片だけを残す（全部捨てることになる場合は元の並びを使うので、空にはならない）。整形前は2000件中228件にナビの断片が残っていた。
